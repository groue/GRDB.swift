...
class RowAsDictionaryLiteralConvertibleTests: GRDBTestCase {
    ...
    func testRowValueAtIndex() {
        ...
        let aIndex = row.startIndex.distanceTo(row.indexOf({ (column, value) in column == "a" })!)
        let bIndex = row.startIndex.distanceTo(row.indexOf({ (column, value) in column == "b" })!)
        let cIndex = row.startIndex.distanceTo(row.indexOf({ (column, value) in column == "c" })!)
    ...
    func testRowDatabaseValueAtIndex() {
        assertNoError {
            ...
            let nullIndex = row.startIndex.distanceTo(row.indexOf({ (column, value) in column == "null" })!)
            let int64Index = row.startIndex.distanceTo(row.indexOf({ (column, value) in column == "int64" })!)
            let doubleIndex = row.startIndex.distanceTo(row.indexOf({ (column, value) in column == "double" })!)
