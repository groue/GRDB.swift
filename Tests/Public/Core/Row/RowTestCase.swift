...
class RowTestCase: GRDBTestCase {
    ...
    func assertRowConvertedValueEqual<T: DatabaseValueConvertible & Equatable>(_ row: Row, index: Int, value: T) {
        // form 1
        XCTAssertEqual(row.value(atIndex: index) as T, value)
    ...
    func assertRowConvertedValueEqual<T: DatabaseValueConvertible & Equatable>(_ row: Row, name: String, value: T) {
        // form 1
        XCTAssertEqual(row.value(named: name) as T, value)
    ...
    func assertRowConvertedValueEqual<T: DatabaseValueConvertible & Equatable>(_ row: Row, column: Column, value: T) {
        // form 1
        XCTAssertEqual(row.value(column) as T, value)
