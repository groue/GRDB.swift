...
private struct Name: DatabaseValueConvertible {
    ...
    static func fromDatabaseValue(_ dbValue: DatabaseValue) -> Name? {
        ...
        return Name(rawValue: rawValue, fast: false)
    }
}

extension Name: StatementColumnConvertible {
    init(sqliteStatement: SQLiteStatement, index: Int32) {
        self.init(rawValue: String(sqliteStatement: sqliteStatement, index: index), fast: true)
    }
}

class ValueObservationStatementColumnConvertibleTests: GRDBTestCase {
    func testAll() throws {
