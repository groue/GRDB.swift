...
class TransactionObserverSavepointsTests: GRDBTestCase {
    ...
    func testSavepointAsTransaction() throws {
        ...
        try dbQueue.writeWithoutTransaction { db in
            try db.execute(rawSQL: "CREATE TABLE items1 (id INTEGER PRIMARY KEY)")
            try db.execute(rawSQL: "CREATE TABLE items2 (id INTEGER PRIMARY KEY)")
            try db.execute(rawSQL: "SAVEPOINT sp1")
            XCTAssertTrue(db.isInsideTransaction)
            try db.execute(rawSQL: "INSERT INTO items1 (id) VALUES (NULL)")
            XCTAssertEqual(observer.events.count, 0)
            try db.execute(rawSQL: "INSERT INTO items2 (id) VALUES (NULL)")
            XCTAssertEqual(observer.events.count, 0)
            try db.execute(rawSQL: "RELEASE SAVEPOINT sp1")
            XCTAssertFalse(db.isInsideTransaction)
            
            XCTAssertEqual(try Int.fetchOne(db, rawSQL: "SELECT COUNT(*) FROM items1"), 1)
            XCTAssertEqual(try Int.fetchOne(db, rawSQL: "SELECT COUNT(*) FROM items2"), 1)
        }
    ...
    func testSavepointWithIdenticalName() throws {
        ...
        try dbQueue.writeWithoutTransaction { db in
            try db.execute(rawSQL: "CREATE TABLE items1 (id INTEGER PRIMARY KEY)")
            try db.execute(rawSQL: "CREATE TABLE items2 (id INTEGER PRIMARY KEY)")
            try db.execute(rawSQL: "CREATE TABLE items3 (id INTEGER PRIMARY KEY)")
            try db.execute(rawSQL: "CREATE TABLE items4 (id INTEGER PRIMARY KEY)")
            try db.execute(rawSQL: "BEGIN TRANSACTION")
            try db.execute(rawSQL: "INSERT INTO items1 (id) VALUES (NULL)")
            XCTAssertEqual(observer.events.count, 1)
            try db.execute(rawSQL: "SAVEPOINT sp1")
            ...
            XCTAssertEqual(observer.events.count, 1)
            try db.execute(rawSQL: "SAVEPOINT sp1")
            try db.execute(rawSQL: "INSERT INTO items3 (id) VALUES (NULL)")
            XCTAssertEqual(observer.events.count, 1)
            try db.execute(rawSQL: "RELEASE SAVEPOINT sp1")
            XCTAssertEqual(observer.events.count, 1)
            try db.execute(rawSQL: "RELEASE SAVEPOINT sp1")
            XCTAssertEqual(observer.events.count, 3)
            try db.execute(rawSQL: "INSERT INTO items4 (id) VALUES (NULL)")
            XCTAssertEqual(observer.events.count, 4)
            try db.execute(rawSQL: "COMMIT")
            
            XCTAssertEqual(try Int.fetchOne(db, rawSQL: "SELECT COUNT(*) FROM items1"), 1)
            XCTAssertEqual(try Int.fetchOne(db, rawSQL: "SELECT COUNT(*) FROM items2"), 1)
            XCTAssertEqual(try Int.fetchOne(db, rawSQL: "SELECT COUNT(*) FROM items3"), 1)
            XCTAssertEqual(try Int.fetchOne(db, rawSQL: "SELECT COUNT(*) FROM items4"), 1)
        }
    ...
    func testMultipleRollbackOfSavepoint() throws {
        ...
        try dbQueue.writeWithoutTransaction { db in
            ...
            try db.execute(rawSQL: "INSERT INTO items3 (id) VALUES (NULL)")
            XCTAssertEqual(observer.events.count, 1)
            try db.execute(rawSQL: "ROLLBACK TO SAVEPOINT sp1")
            try db.execute(rawSQL: "INSERT INTO items4 (id) VALUES (NULL)")
            XCTAssertEqual(observer.events.count, 1)
            try db.execute(rawSQL: "ROLLBACK TO SAVEPOINT sp1")
            try db.execute(rawSQL: "INSERT INTO items4 (id) VALUES (NULL)")
            ...
            XCTAssertEqual(try Int.fetchOne(db, rawSQL: "SELECT COUNT(*) FROM items1"), 1)
            XCTAssertEqual(try Int.fetchOne(db, rawSQL: "SELECT COUNT(*) FROM items2"), 0)
            XCTAssertEqual(try Int.fetchOne(db, rawSQL: "SELECT COUNT(*) FROM items3"), 0)
            XCTAssertEqual(try Int.fetchOne(db, rawSQL: "SELECT COUNT(*) FROM items4"), 1)
        }
    ...
    func testRollbackNonNestedSavepointInsideTransaction() throws {
        ...
        try dbQueue.writeWithoutTransaction { db in
            ...
            try db.execute(rawSQL: "INSERT INTO items2 (id) VALUES (NULL)")
            XCTAssertEqual(observer.events.count, 1)
            try db.execute(rawSQL: "SAVEPOINT sp2")
            try db.execute(rawSQL: "INSERT INTO items3 (id) VALUES (NULL)")
            XCTAssertEqual(observer.events.count, 1)
            try db.execute(rawSQL: "RELEASE SAVEPOINT sp2")
            XCTAssertEqual(observer.events.count, 1)
            try db.execute(rawSQL: "ROLLBACK TO SAVEPOINT sp1")
            XCTAssertEqual(observer.events.count, 1)
            try db.execute(rawSQL: "INSERT INTO items4 (id) VALUES (NULL)")
