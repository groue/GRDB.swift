...
class DatabaseQueueTests: GRDBTestCase {
    ...
    func testAddRemoveFunction() throws {
        ...
        do {
            ...
            XCTAssertEqual(
                error.description.lowercased(),
                "sqlite error 1: no such function: succ - while executing `select succ(1)`")
        }
    }
    ...
    func testAddRemoveCollation() throws {
        ...
        do {
            try dbQueue.inDatabase { db in
                try db.execute(
                    sql: "CREATE TABLE files_fail (name TEXT COLLATE TEST_COLLATION_FOO)")
                XCTFail("Expected Error")
            }
            XCTFail("Expected Error")
        } catch let error as DatabaseError {
            ...
            XCTAssertEqual(error.resultCode, .SQLITE_ERROR)
            XCTAssertEqual(
                error.message!.lowercased(), "no such collation sequence: test_collation_foo")  // lowercaseString: accept multiple SQLite version
            XCTAssertEqual(
                error.sql!, "CREATE TABLE files_fail (name TEXT COLLATE TEST_COLLATION_FOO)")
            XCTAssertEqual(
                error.description.lowercased(),
                "sqlite error 1: no such collation sequence: test_collation_foo - while executing `create table files_fail (name text collate test_collation_foo)`"
            )
        }
    }
    ...
    func
        test_busy_timeout_does_not_prevent_SQLITE_BUSY_when_write_lock_is_acquired_by_other_connection()
        throws
    {
    ...
    // See <https://github.com/groue/GRDB.swift/issues/1483>
    func
        test_busy_timeout_does_not_prevent_SQLITE_BUSY_when_write_lock_was_acquired_by_other_connection()
        throws
    {
        var configuration = dbConfiguration!
        configuration.journalMode = .wal
