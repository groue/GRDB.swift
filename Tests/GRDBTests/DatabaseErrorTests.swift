    ...
    func testDatabaseErrorInTransaction() throws {
        ...
        do {
            try dbQueue.inTransaction { db in
                ...
                clearSQLQueries()
                try db.execute(
                    sql: "INSERT INTO pets (masterId, name) VALUES (?, ?)", arguments: [1, "Bobby"])
                XCTFail()
                return .commit
            }
            ...
            XCTAssertEqual(
                ...
                "sqlite error 19: foreign key constraint failed - while executing `insert into pets (masterid, name) values (?, ?)`"
            )
            XCTAssertEqual(
                error.expandedDescription.lowercased(),
                "sqlite error 19: foreign key constraint failed - while executing `insert into pets (masterid, name) values (?, ?)` with arguments [1, \"bobby\"]"
            )
            ...
            try dbQueue.writeWithoutTransaction { db in
                do {
                    try db.inSavepoint {
                        ...
                        clearSQLQueries()
                        try db.execute(
                            sql: "INSERT INTO pets (masterId, name) VALUES (?, ?)",
                            arguments: [1, "Bobby"])
                        XCTFail()
                        return .commit
                    }
    ...
    func testDatabaseErrorThrownByExecuteMultiStatementContainSQL() throws {
        ...
        try dbQueue.inDatabase { db in
            do {
                ...
                XCTAssertEqual(
                    error.description.lowercased(),
                    "sqlite error 19: foreign key constraint failed - while executing `insert into pets (masterid, name) values (1, 'bobby')`"
                )
                XCTAssertEqual(
                    error.expandedDescription.lowercased(),
                    "sqlite error 19: foreign key constraint failed - while executing `insert into pets (masterid, name) values (1, 'bobby')`"
