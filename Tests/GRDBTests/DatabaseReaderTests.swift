...
class DatabaseReaderTests: GRDBTestCase {
    ...
    func testAsyncRead() throws {
        func test(_ dbReader: some DatabaseReader) throws {
            ...
            dbReader.asyncRead { dbResult in
                ...
                semaphore.wait()
                do {
                    try countMutex.store(
                        Int.fetchOne(dbResult.get(), sql: "SELECT COUNT(*) FROM sqlite_master"))
                } catch {
    ...
    func testAsyncReadPreventsDatabaseModification() throws {
        func test(_ dbReader: some DatabaseReader) throws {
            ...
            dbReader.asyncRead { dbResult in
                ...
                semaphore.wait()
                do {
                    try dbResult.get().execute(
                        sql: "CREATE TABLE testAsyncReadPreventsDatabaseModification (a)")
                    XCTFail("Expected error")
    ...
    func
        test_successful_read_is_not_cancelled_by_Task_cancellation_performed_after_database_access()
        async throws
    {
        func test(_ dbReader: some DatabaseReader) async throws {
            let semaphore = AsyncSemaphore(value: 0)
    ...
    func
        test_statement_execution_from_read_is_cancelled_by_Task_cancellation_performed_after_database_access()
        async throws
    {
        func test(_ dbReader: some DatabaseReader) async throws {
            let semaphore = AsyncSemaphore(value: 0)
    ...
    func
        test_cursor_iteration_from_read_is_interrupted_by_Task_cancellation_performed_after_database_access()
        async throws
    {
        func test(_ dbReader: some DatabaseReader) async throws {
            let semaphore = AsyncSemaphore(value: 0)
            ...
            let task = Task {
                await semaphore.wait()
                try await dbReader.read { db in
                    let cursor = try Int.fetchCursor(
                        db,
                        sql: """
    ...
    func
        test_successful_unsafeRead_is_not_cancelled_by_Task_cancellation_performed_after_database_access()
        async throws
    {
        func test(_ dbReader: some DatabaseReader) async throws {
            let semaphore = AsyncSemaphore(value: 0)
    ...
    func
        test_statement_execution_from_unsafeRead_is_cancelled_by_Task_cancellation_performed_after_database_access()
        async throws
    {
        func test(_ dbReader: some DatabaseReader) async throws {
            let semaphore = AsyncSemaphore(value: 0)
    ...
    func
        test_cursor_iteration_from_unsafeRead_is_interrupted_by_Task_cancellation_performed_after_database_access()
        async throws
    {
        func test(_ dbReader: some DatabaseReader) async throws {
            let semaphore = AsyncSemaphore(value: 0)
