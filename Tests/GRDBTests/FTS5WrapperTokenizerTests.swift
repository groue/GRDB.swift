...
class FTS5WrapperTokenizerTests: GRDBTestCase {
    ...
    func testLatinAsciiTokenizer() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            XCTAssertEqual(try Int.fetchOne(db, rawSQL: "SELECT COUNT(*) FROM documents WHERE documents MATCH ?", arguments: ["aimé \u{FB01}délité Encyclopædia Großmann Diyarbakır"]), 1)
            XCTAssertEqual(try Int.fetchOne(db, rawSQL: "SELECT COUNT(*) FROM documents WHERE documents MATCH ?", arguments: ["aime fidelite encyclopaedia grossmann diyarbakir"]), 0)
            XCTAssertEqual(try Int.fetchOne(db, rawSQL: "SELECT COUNT(*) FROM documents WHERE documents MATCH ?", arguments: ["aime\u{0301} \u{FB01}de\u{0301}lite\u{0301} Encyclopædia Großmann Diyarbakır"]), 1)
        ...
        try dbQueue.inDatabase { db in
            ...
            XCTAssertEqual(try Int.fetchOne(db, rawSQL: "SELECT COUNT(*) FROM documents WHERE documents MATCH ?", arguments: ["aimé \u{FB01}délité Encyclopædia Großmann Diyarbakır"]), 1)
            XCTAssertEqual(try Int.fetchOne(db, rawSQL: "SELECT COUNT(*) FROM documents WHERE documents MATCH ?", arguments: ["aime fidelite encyclopaedia grossmann diyarbakir"]), 1)
            XCTAssertEqual(try Int.fetchOne(db, rawSQL: "SELECT COUNT(*) FROM documents WHERE documents MATCH ?", arguments: ["aime\u{0301} \u{FB01}de\u{0301}lite\u{0301} Encyclopædia Großmann Diyarbakır"]), 1)
