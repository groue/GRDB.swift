...
class ValueObservationRowTests: GRDBTestCase {
    func testAll() throws {
        ...
        try assertValueObservation(
            ValueObservation.trackingConstantRegion {
                // Compare Row descriptions in order to avoid concurrency warnings
                try request.fetchAll($0).map { $0.description }
            },
            records: [
                [],
                [#"[id:1 name:"foo"]"#],
    ...
    func testOne() throws {
        ...
        try assertValueObservation(
            ValueObservation.trackingConstantRegion {
                // Compare Row description in order to avoid concurrency warnings
                try request.fetchOne($0).map { $0.description }
            },
            records: [
                nil,
                #"[id:1 name:"foo"]"#,
    ...
    func testFTS4Observation() throws {
        try assertValueObservation(
            ValueObservation.trackingConstantRegion {
                // Compare Row descriptions in order to avoid concurrency warnings
                try SQLRequest<Row>(sql: "SELECT * FROM ft_documents")
                    .fetchAll($0)
                    .map { $0.description }
            },
            records: [
