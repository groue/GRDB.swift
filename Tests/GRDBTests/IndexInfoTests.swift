...
class IndexInfoTests: GRDBTestCase {
    ...
    func testIndexes() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            do {
                try db.execute(rawSQL: "CREATE TABLE citizenships (year INTEGER, personId INTEGER NOT NULL, countryIsoCode TEXT NOT NULL, PRIMARY KEY (personId, countryIsoCode))")
                try db.execute(rawSQL: "CREATE INDEX citizenshipsOnYear ON citizenships(year)")
