...
class ValueObservationTests: GRDBTestCase {
    ...
    // MARK: - Snapshot Observation
        ...
        func testDatabaseSnapshotPoolObservation() throws {
            ...
            try XCTAssertEqual(
                results[0].read { try Int.fetchOne($0, sql: "SELECT COUNT(*) FROM t")! }, 0)
            try XCTAssertEqual(
                results[1].read { try Int.fetchOne($0, sql: "SELECT COUNT(*) FROM t")! }, 1)
        }
    #endif
                        ...
                        AnyValueReducer<Void, Void>(
                            ...
                            value: { _ in
                                shouldStopObservationMutex.withLock { shouldStopObservation in
                                    if shouldStopObservation {
                                        cancellable =
                                            nil /* deallocation right before notification */
                                    }
                                    shouldStopObservation = true
                                }
                                return ()
    ...
    @MainActor func test_mainActor_observation() throws {
        ...
        func testAsyncAwait_values_prefix() async throws {
            func test(_ writer: some DatabaseWriter) async throws {
                ...
                let task = Task { () -> [Int] in
                    ...
                    for try await count in try observation.values(in: writer).prefix(while: {
                        $0 <= 3
                    }) {
                        counts.append(count)
                        try await writer.write {
        ...
        func testManyObservations() throws {
            ...
            #if GRDBCIPHER
                if Database.sqliteLibVersionNumber <= 3_020_001 {
            ...
            func test(
                _ writer: some DatabaseWriter, scheduling scheduler: some ValueObservationScheduler
            ) throws {
                try writer.write {
                    try $0.execute(sql: "CREATE TABLE t(id INTEGER PRIMARY KEY AUTOINCREMENT)")
                }
                ...
                for _ in 0..<observationCount {
                    let cancellable = observation.start(in: writer, scheduling: scheduler) {
                        error in
                        XCTFail("Unexpected error: \(error)")
                    } onChange: { count in
                        if count == 0 {
            ...
            try Test(test).runAtTemporaryDatabasePath {
                try (DatabaseQueue(path: $0), .async(onQueue: .main))
            }
            try Test(test).runAtTemporaryDatabasePath {
                try (DatabasePool(path: $0), .async(onQueue: .main))
            }
        }
        ...
        func testIssue1362() throws {
            func test(_ writer: some DatabaseWriter) throws {
                try writer.write {
                    try $0.execute(sql: "CREATE TABLE s(id INTEGER PRIMARY KEY AUTOINCREMENT)")
                }
                var cancellables = [AnyDatabaseCancellable]()

                // Start an observation and wait until it has installed its
                // transaction observer.
                let installedExpectation = expectation(
                    description: "transaction observer installed")
                let finalExpectation = expectation(description: "final value")
                let initialObservation = ValueObservation.trackingConstantRegion(
                    Table("s").fetchCount)
    ...
    func testIssue1383_createWal() throws {
        ...
        try? FileManager.default.removeItem(
            at: url.deletingLastPathComponent().appendingPathComponent("Issue1383.sqlite-wal"))
        try? FileManager.default.removeItem(
            at: url.deletingLastPathComponent().appendingPathComponent("Issue1383.sqlite-shm"))
