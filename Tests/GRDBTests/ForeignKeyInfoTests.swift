...
class ForeignKeyInfoTests: GRDBTestCase {
    ...
    func testForeignKeys() throws {
        ...
        try dbQueue.inDatabase { db in
            try db.execute("CREATE TABLE parents1 (id PRIMARY KEY)")
            try db.execute("CREATE TABLE parents2 (a, b, PRIMARY KEY (a,b))")
            try db.execute("CREATE TABLE children1 (parentId REFERENCES parents1)")
            try db.execute("CREATE TABLE children2 (parentId1 REFERENCES parents1, parentId2 REFERENCES parents1)")
            try db.execute("CREATE TABLE children3 (parentA, parentB, FOREIGN KEY (parentA, parentB) REFERENCES parents2)")
