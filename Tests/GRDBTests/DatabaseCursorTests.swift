...
class DatabaseCursorTests: GRDBTestCase {
    ...
    func testNextReturnsNilAfterExhaustion() throws {
        ...
        try dbQueue.inDatabase { db in
            do {
                let cursor = try Int.fetchCursor(db, rawSQL: "SELECT 1 WHERE 0")
                XCTAssert(try cursor.next() == nil) // end
                XCTAssert(try cursor.next() == nil) // past the end
            }
            do {
                let cursor = try Int.fetchCursor(db, rawSQL: "SELECT 1")
                XCTAssertEqual(try cursor.next()!,  1)
                XCTAssert(try cursor.next() == nil) // end
                XCTAssert(try cursor.next() == nil) // past the end
            }
            do {
                let cursor = try Int.fetchCursor(db, rawSQL: "SELECT 1 UNION ALL SELECT 2")
                XCTAssertEqual(try cursor.next()!, 1)
    ...
    func testStepError() throws {
        ...
        try dbQueue.inDatabase { db in
            let cursor = try Int.fetchCursor(db, rawSQL: "SELECT throw()")
            do {
                _ = try cursor.next()
                XCTFail()
