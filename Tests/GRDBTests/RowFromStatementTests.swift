...
class RowFromStatementTests : RowTestCase {
    ...
    func testRowAsSequence() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            let rows = try Row.fetchCursor(db, rawSQL: "SELECT * FROM ints")
    ...
    func testDataNoCopy() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            let emptyData = Data()
            var rowFetched = false
            let rows = try Row.fetchCursor(db, rawSQL: "SELECT ? AS a, ? AS b, ? AS c", arguments: [data, emptyData, nil])
            while let row = try rows.next() {
                rowFetched = true
    ...
    func testRowDatabaseValueAtIndex() throws {
        ...
        try dbQueue.inDatabase { db in
            var rowFetched = false
            let rows = try Row.fetchCursor(db, rawSQL: "SELECT NULL, 1, 1.1, 'foo', x'53514C697465'")
            while let row = try rows.next() {
                rowFetched = true
    ...
    func testRowDatabaseValueNamed() throws {
        ...
        try dbQueue.inDatabase { db in
            var rowFetched = false
            let rows = try Row.fetchCursor(db, rawSQL: "SELECT NULL AS \"null\", 1 AS \"int64\", 1.1 AS \"double\", 'foo' AS \"string\", x'53514C697465' AS \"blob\"")
            while let row = try rows.next() {
                rowFetched = true
    ...
    func testRowColumnNames() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            try db.execute(rawSQL: "INSERT INTO ints (a,b,c) VALUES (0, 1, 2)")
            var rowFetched = false
            let rows = try Row.fetchCursor(db, rawSQL: "SELECT a, b, c FROM ints")
            while let row = try rows.next() {
                rowFetched = true
    ...
    func testRowIsCaseInsensitive() throws {
        ...
        try dbQueue.inDatabase { db in
            var rowFetched = false
            let rows = try Row.fetchCursor(db, rawSQL: "SELECT 'foo' AS nAmE")
            while let row = try rows.next() {
                rowFetched = true
    ...
    func testRowIsCaseInsensitiveAndReturnsLeftmostMatchingColumn() throws {
        ...
        try dbQueue.inDatabase { db in
            var rowFetched = false
            let rows = try Row.fetchCursor(db, rawSQL: "SELECT 1 AS name, 2 AS NAME")
            while let row = try rows.next() {
                rowFetched = true
    ...
    func testRowHasColumnIsCaseInsensitive() throws {
        ...
        try dbQueue.inDatabase { db in
            var rowFetched = false
            let rows = try Row.fetchCursor(db, rawSQL: "SELECT 'foo' AS nAmE, 1 AS foo")
            while let row = try rows.next() {
                rowFetched = true
    ...
    func testDatabaseCursorMap() throws {
        let dbQueue = try makeDatabaseQueue()
        try dbQueue.inDatabase { db in
            let cursor = try Row.fetchCursor(db, rawSQL: "SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3")
    ...
    func testDescription() throws {
        let dbQueue = try makeDatabaseQueue()
        try dbQueue.inDatabase { db in
            let rows = try Row.fetchCursor(db, rawSQL: "SELECT NULL AS \"null\", 1 AS \"int\", 1.1 AS \"double\", 'foo' AS \"string\", x'53514C697465' AS \"data\"")
            var rowFetched = false
            while let row = try rows.next() {
                rowFetched = true
