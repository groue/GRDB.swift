    ...
    func testFetchCursor() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            do {
                ...
                try test(SQLRequest(sql).fetching(Row.self).fetchCursor(db))
            }
            do {
                ...
                try test(SQLRequest(sql, adapter: adapter).fetching(Row.self).fetchCursor(db))
            }
        }
    }
    ...
    func testFetchCursorStepFailure() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            do {
                ...
                try test(SQLRequest(sql).fetching(Row.self).fetchCursor(db), sql: sql)
            }
            do {
                ...
                try test(SQLRequest(sql, adapter: adapter).fetching(Row.self).fetchCursor(db), sql: sql)
            }
        }
    }
    ...
    func testFetchAll() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            do {
                ...
                try test(SQLRequest(sql).fetching(Row.self).fetchAll(db))
            }
            do {
                let sql = "SELECT 0 AS firstName, 'Arthur' AS firstName, 'Martin' AS lastName UNION ALL SELECT 0, 'Barbara', 'Gourde'"
                let statement = try db.makeSelectStatement(sql)
                ...
                try test(SQLRequest(sql, adapter: adapter).fetching(Row.self).fetchAll(db))
            }
        }
    }
    ...
    func testFetchAllStepFailure() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            do {
                ...
                try test(SQLRequest(sql).fetching(Row.self).fetchAll(db), sql: sql)
            }
            do {
                ...
                try test(SQLRequest(sql, adapter: adapter).fetching(Row.self).fetchAll(db), sql: sql)
            }
        }
    }
        ...
        try dbQueue.inDatabase { db in
            do {
                ...
                do {
                    ...
                    try test(SQLRequest(sql).fetching(Row.self).fetchOne(db))
                }
                do {
                    ...
                    try test(SQLRequest(sql, adapter: adapter).fetching(Row.self).fetchOne(db))
                }
            }
            do {
                func test(_ row: Row?) {
    ...
    func testFetchOneStepFailure() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            do {
                ...
                try test(SQLRequest(sql).fetching(Row.self).fetchOne(db), sql: sql)
            }
            do {
                let sql = "SELECT 0, throw()"
                let adapter = SuffixRowAdapter(fromIndex: 1)
                ...
                try test(SQLRequest(sql, adapter: adapter).fetching(Row.self).fetchOne(db), sql: sql)
            }
        }
    }
