...
class FTS4TableBuilderTests: GRDBTestCase {
    ...
    // Issue 620
    func testObservationSynchronized1() throws {
        let dbQueue = try makeDatabaseQueue()
        try dbQueue.write { db in
            try db.create(table: "documents") { t in
                t.column("id", .integer).primaryKey()
        ...
        let request = SQLRequest<Row>(sql: "SELECT * FROM documents")
        let observation = ValueObservation.tracking(value: request.fetchAll)
        let observer = observation.start(
            in: dbQueue,
            onError: { error in
        ...
        try withExtendedLifetime(observer) {
            ...
            waitForExpectations(timeout: 1, handler: nil)
            XCTAssertEqual(rows, [[], [["id":1, "content":"foo"]]])
        }
    }
    
    // Issue 620
    func testObservationSynchronized2() throws {
        let dbQueue = try makeDatabaseQueue()
        try dbQueue.write { db in
            try db.create(table: "documents") { t in
                t.column("id", .integer).primaryKey()
