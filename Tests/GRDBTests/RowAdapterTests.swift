...
class AdapterRowTests : RowTestCase {
    ...
    func testRowAsSequence() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            let row = try Row.fetchOne(db, rawSQL: "SELECT 0 AS basea, 'XXX' AS extra, 1 AS baseb, 2 as basec", adapter: adapter)!
    ...
    func testDataNoCopy() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            let emptyData = Data()
            let adapter = ColumnMapping(["a": "basea", "b": "baseb", "c": "basec"])
            let row = try Row.fetchOne(db, rawSQL: "SELECT ? AS basea, ? AS baseb, ? AS basec", arguments: [data, emptyData, nil], adapter: adapter)!
    ...
    func testRowDatabaseValueAtIndex() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            let row = try Row.fetchOne(db, rawSQL: "SELECT NULL AS basenull, 'XXX' AS extra, 1 AS baseint64, 1.1 AS basedouble, 'foo' AS basestring, x'53514C697465' AS baseblob", adapter: adapter)!
    ...
    func testRowIsCaseInsensitive() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            let row = try Row.fetchOne(db, rawSQL: "SELECT 'foo' AS basenAmE, 'XXX' AS extra", adapter: adapter)!
    ...
    func testRowIsCaseInsensitiveAndReturnsLeftmostMatchingColumn() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            let row = try Row.fetchOne(db, rawSQL: "SELECT 1 AS basename, 'XXX' AS extra, 2 AS baseNAME", adapter: adapter)!
    ...
    func testRowAdapterIsCaseInsensitiveAndPicksLeftmostBaseColumn() throws {
        ...
        try dbQueue.inDatabase { db in
            let adapter = ColumnMapping(["name": "baseNaMe"])
            let row = try Row.fetchOne(db, rawSQL: "SELECT 1 AS basename, 2 AS baseNaMe, 3 AS BASENAME", adapter: adapter)!
    ...
    func testMissingColumn() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            let row = try Row.fetchOne(db, rawSQL: "SELECT 1 AS name, 'foo' AS missing", adapter: adapter)!
    ...
    func testRowHasColumnIsCaseInsensitive() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            let row = try Row.fetchOne(db, rawSQL: "SELECT 'foo' AS basenAmE, 'XXX' AS extra, 1 AS basefoo", adapter: adapter)!
    ...
    func testScopes() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            let row = try Row.fetchOne(db, rawSQL: "SELECT 1 AS id1, 'foo1' AS val1, 2 as id2, 'foo2' AS val2", adapter: adapter)!
        ...
        try dbQueue.inDatabase { db in
            let adapter = ColumnMapping(["id": "id0", "val": "val0"])
                .addingScopes([
                    "sub1": ColumnMapping(["id": "id1", "val": "val1"]),
                    "sub2": ColumnMapping(["id": "id2", "val": "val2"])])
            let row = try Row.fetchOne(db, rawSQL: "SELECT 0 AS id0, 'foo0' AS val0, 1 AS id1, 'foo1' AS val1, 2 as id2, 'foo2' AS val2", adapter: adapter)!
    ...
    func testSuffixAdapter() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            do {
                let row = try Row.fetchOne(db, rawSQL: sql, adapter: SuffixRowAdapter(fromIndex:0))!
            ...
            do {
                let row = try Row.fetchOne(db, rawSQL: sql, adapter: SuffixRowAdapter(fromIndex:1))!
            ...
            do {
                let row = try Row.fetchOne(db, rawSQL: sql, adapter: SuffixRowAdapter(fromIndex:3))!
                XCTAssertEqual(Array(row.columnNames), ["a3"])
                XCTAssertEqual(Array(row.databaseValues), [3.databaseValue])
            }
            do {
                let row = try Row.fetchOne(db, rawSQL: sql, adapter: SuffixRowAdapter(fromIndex:4))!
    ...
    func testSuffixAdapterIndexesAreIndependentFromScopes() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            let row = try Row.fetchOne(db, rawSQL: "SELECT 0 AS a0, 1 AS a1, 2 AS a2, 3 AS a3", adapter: adapter)!
    ...
    func testRangeAdapterWithCountableRange() throws {
        ...
        try dbQueue.inDatabase { db in
            let sql = "SELECT 0 AS a0, 1 AS a1, 2 AS a2, 3 AS a3"
            do {
                let row = try Row.fetchOne(db, rawSQL: sql, adapter: RangeRowAdapter(0..<0))!
                XCTAssertEqual(Array(row.columnNames), [])
                XCTAssertEqual(Array(row.databaseValues), [])
            }
            do {
                let row = try Row.fetchOne(db, rawSQL: sql, adapter: RangeRowAdapter(0..<1))!
                XCTAssertEqual(Array(row.columnNames), ["a0"])
                XCTAssertEqual(Array(row.databaseValues), [0.databaseValue])
            }
            do {
                let row = try Row.fetchOne(db, rawSQL: sql, adapter: RangeRowAdapter(1..<3))!
                XCTAssertEqual(Array(row.columnNames), ["a1", "a2"])
                XCTAssertEqual(Array(row.databaseValues), [1.databaseValue, 2.databaseValue])
            }
            do {
                let row = try Row.fetchOne(db, rawSQL: sql, adapter: RangeRowAdapter(0..<4))!
                XCTAssertEqual(Array(row.columnNames), ["a0", "a1", "a2", "a3"])
                XCTAssertEqual(Array(row.databaseValues), [0.databaseValue, 1.databaseValue, 2.databaseValue, 3.databaseValue])
            }
            do {
                let row = try Row.fetchOne(db, rawSQL: sql, adapter: RangeRowAdapter(4..<4))!
                XCTAssertEqual(Array(row.columnNames), [])
                XCTAssertEqual(Array(row.databaseValues), [])
            }
        }
    ...
    func testRangeAdapterWithCountableClosedRange() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            do {
                let row = try Row.fetchOne(db, rawSQL: sql, adapter: RangeRowAdapter(1...2))!
                XCTAssertEqual(Array(row.columnNames), ["a1", "a2"])
                XCTAssertEqual(Array(row.databaseValues), [1.databaseValue, 2.databaseValue])
            }
            do {
                let row = try Row.fetchOne(db, rawSQL: sql, adapter: RangeRowAdapter(0...3))!
                XCTAssertEqual(Array(row.columnNames), ["a0", "a1", "a2", "a3"])
                XCTAssertEqual(Array(row.databaseValues), [0.databaseValue, 1.databaseValue, 2.databaseValue, 3.databaseValue])
            }
            do {
                let row = try Row.fetchOne(db, rawSQL: sql, adapter: RangeRowAdapter(3...3))!
                XCTAssertEqual(Array(row.columnNames), ["a3"])
                XCTAssertEqual(Array(row.databaseValues), [3.databaseValue])
            }
        }
    ...
    func testCopy() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            let baseRows = try Row.fetchCursor(db, rawSQL: "SELECT 0 AS basea, 'XXX' AS extra, 1 AS baseb, 2 as basec", adapter: adapter)
    ...
    func testEqualityComparesScopes() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            let row1 = try Row.fetchOne(db, rawSQL: "SELECT 0 AS basea, 'XXX' AS extra, 1 AS baseb, 1 AS baseb2, 2 as basec", adapter: adapter1)!
            let row2 = try Row.fetchOne(db, rawSQL: "SELECT 0 AS basea, 'XXX' AS extra, 1 AS baseb, 1 AS baseb2, 2 as basec", adapter: adapter2)!
            let row3 = try Row.fetchOne(db, rawSQL: "SELECT 0 AS basea, 'XXX' AS extra, 1 AS baseb, 1 AS baseb2, 2 as basec", adapter: adapter3)!
            let row4 = try Row.fetchOne(db, rawSQL: "SELECT 0 AS basea, 'XXX' AS extra, 1 AS baseb, 1 AS baseb2, 2 as basec", adapter: adapter4)!
            let row5 = try Row.fetchOne(db, rawSQL: "SELECT 0 AS basea, 'XXX' AS extra, 1 AS baseb, 1 AS baseb2, 2 as basec", adapter: adapter5)!
    ...
    func testEqualityWithNonMappedRow() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            let mappedRow1 = try Row.fetchOne(db, rawSQL: "SELECT 1 AS baseid, 'XXX' AS extra, 'foo' AS baseval", adapter: adapter)!
            let mappedRow2 = try Row.fetchOne(db, rawSQL: "SELECT 'foo' AS baseval, 'XXX' AS extra, 1 AS baseid", adapter: adapter)!
            let nonMappedRow1 = try Row.fetchOne(db, rawSQL: "SELECT 1 AS id, 'foo' AS val")!
            let nonMappedRow2 = try Row.fetchOne(db, rawSQL: "SELECT 'foo' AS val, 1 AS id")!
    ...
    func testEmptyMapping() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            let row = try Row.fetchOne(db, rawSQL: "SELECT 'foo' AS foo", adapter: adapter)!
