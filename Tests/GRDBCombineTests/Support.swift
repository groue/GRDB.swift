    ...
    final class Test<Context> {
        ...
        @discardableResult
        func runInTemporaryDirectory(context: (_ directoryURL: URL) throws -> Context) throws
            -> Self
        {
            for i in 1...repeatCount {
    ...
    final class AsyncTest<Context> {
        ...
        @discardableResult
        func runInTemporaryDirectory(context: (_ directoryURL: URL) async throws -> Context)
            async throws -> Self
        {
            for i in 1...repeatCount {
                let directoryURL = URL(fileURLWithPath: NSTemporaryDirectory())
        ...
        @discardableResult
        func runAtTemporaryDatabasePath(context: (_ path: String) async throws -> Context)
            async throws -> Self
        {
    ...
    public func assertFailure<Failure, ExpectedFailure>(
        ...
        if case .failure(let error) = completion, let expectedError = error as? ExpectedFailure {
            test(expectedError)
        } else {
