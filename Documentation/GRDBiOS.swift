...
final public class FetchedRecordsController<Record : RowConvertible> {
    public convenience init(_ databaseWriter: DatabaseWriter, sql: String, arguments: GRDB.StatementArguments? = default, queue: dispatch_queue_t = default, isSameRecord: ((Record, Record) -> Bool)? = default)
    public convenience init<T>(_ databaseWriter: DatabaseWriter, request: GRDB.FetchRequest<T>, queue: dispatch_queue_t = default, isSameRecord: ((Record, Record) -> Bool)? = default)
    public func performFetch()
    weak public var delegate: FetchedRecordsControllerDelegate?
    public let databaseWriter: DatabaseWriter
    public let queue: dispatch_queue_t
    public func setRequest<T>(request: GRDB.FetchRequest<T>)
    public func setRequest(sql sql: String, arguments: GRDB.StatementArguments? = default)
    ...
    public var sections: [GRDB.FetchedRecordsSectionInfo<Record>] { get }
}

extension FetchedRecordsController where Record : MutablePersistable {
    public convenience init(_ databaseWriter: DatabaseWriter, sql: String, arguments: GRDB.StatementArguments? = default, queue: dispatch_queue_t = default, compareRecordsByPrimaryKey: Bool)
    public convenience init<U>(_ databaseWriter: DatabaseWriter, request: GRDB.FetchRequest<U>, queue: dispatch_queue_t = default, compareRecordsByPrimaryKey: Bool)
}

public protocol FetchedRecordsControllerDelegate : class {
    public func controllerWillChangeRecords<T>(controller: GRDB.FetchedRecordsController<T>)
    public func controller<T>(controller: GRDB.FetchedRecordsController<T>, didChangeRecord record: T, withEvent event: GRDB.FetchedRecordsEvent)
    public func controllerDidChangeRecords<T>(controller: GRDB.FetchedRecordsController<T>)
}
...
public enum FetchedRecordsEvent {
    ...
    case Move(indexPath: NSIndexPath, newIndexPath: NSIndexPath, changes: [String : GRDB.DatabaseValue])
    case Update(indexPath: NSIndexPath, changes: [String : GRDB.DatabaseValue])
}
