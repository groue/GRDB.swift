...
It is important to put things in the right order. An SQLite database stored on one of your user's device is more important than the Swift code that accesses it. When you ship a new version of your application, all the code can change, but the database remains the same. You may even use another library than GRDB one day :wink:
