...
Most libraries that provide record types want you to subclass a root class: NSManagedObject, Realm.Object, FCModel.
...
Finally, all changes are notified, no matter how they are performed: record methods, raw SQL statements, foreign key cascades, and even SQL triggers.


### Non-blocking database reads

GRDB ships with two ways to access databases, [database queues](https://github.com/groue/GRDB.swift/blob/master/README.md#database-queues) and [pools](https://github.com/groue/GRDB.swift/blob/master/README.md#database-pools).
...
[Database pools](https://github.com/groue/GRDB.swift/blob/master/README.md#database-pools) can lift these unwanted locks. With database pools, reads are generally non-blocking, unless the maximum number of concurrent reads has been reached (and this maximum number can be configured).
...
We can see that the threat of conflicts is *always* left to the application. Core Data provides conflict policies, but no one can pretend that they are easy to use or test. This is an argument for leaving the application 100% responsible for conflict handling, in hope that simple ones can be handled in a simple way. A weaker way to say this is that conflict handling at the library level can wait :sweat_smile:.
...
let exAequoCount: Int = try Player
    .filter(scoreColumn == maximumScore)
    .fetchCount(db)
```
...
[Concurrency Guide]: https://github.com/groue/GRDB.swift/blob/master/README.md#concurrency
[Core Data]: https://developer.apple.com/documentation/coredata
[DatabasePool]: https://github.com/groue/GRDB.swift/blob/master/README.md#database-pools
[Diesel]: http://diesel.rs
[FCModel]: https://github.com/marcoarment/FCModel
[FetchedRecordsController]: https://github.com/groue/GRDB.swift/blob/master/README.md#fetchedrecordscontroller
