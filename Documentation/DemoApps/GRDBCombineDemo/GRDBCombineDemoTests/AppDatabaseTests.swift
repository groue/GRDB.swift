...
class AppDatabaseTests: XCTestCase {
    ...
    func test_playersOrderedByNamePublisher_publishes_well_ordered_players() throws {
        ...
        var players: [Player]?
        let cancellable = appDatabase.playersOrderedByNamePublisher().sink { completion in
    ...
    func test_playersOrderedByScorePublisher_publishes_well_ordered_players() throws {
        ...
        let exp = expectation(description: "Players")
        var players: [Player]?
        let cancellable = appDatabase.playersOrderedByScorePublisher().sink { completion in
            if case let .failure(error) = completion {
                XCTFail("Unexpected error \(error)")
            }
        } receiveValue: {
