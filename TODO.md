...
- [X] Document FetchedRecordsController.trackChanges(fetchAlongside: { db in ... }, ...) 
- [X] Document when it is safe to use unowned references in a FetchedRecordsController's callbacks (answer: when the reference is deallocated from the controller's queue - usually the main queue)
