...
> GRDB offers a `static` as well as `dynamic` library. Choose the default `GRDB` (static) library if you're not sure about the difference. Choose `GRDB-dynamic` if you are going to link it with multiple targets within your app and only wish to link to a shared, dynamic framework once. See [#How to link as dynamic](https://forums.swift.org/t/how-to-link-a-swift-package-as-dynamic/32062)
>
