...
```
github "groue/GRDB.swift" ~> 0.77.0
```
...
You don't know which part of your program needs improvement until you have run a benchmarking tool. **Use [Instruments](https://developer.apple.com/library/ios/documentation/ToolsLanguages/Conceptual/Xcode_Overview/MeasuringPerformance.html).**
...
Particularly: the Array returned by `fetchAll` method, and the sequence returned by `fetch` aren't the same:
...
### Performance tip: use SQL when appropriate
...
for author in authors {
    ...
    author.bookCount = Book(authorIdColumn == author.id).fetchCount(db)
}
```
...
In the example above, if your Author type won't accept an extra bookCount property, *do define and use a different type*.

**Generally, know you database, and use its powerful querying algorithms.**


### Performance tip: use transactions

Individual SQLite statements are much faster when executed inside a transaction. This is because a transaction allows SQLite to postpone writing changes to disk until the final commit:
