...
As a demonstration, we'll rewrite the [above](#splitting-rows-the-request-way) sample code in order to use the decoding code generated by the Swift compiler:
...
> :bulb: In this chapter, we have learned how to use the `Decodable` protocol and its associated `CodingKeys` enum in order to profit from the decoding code generated by the Swift compiler.
