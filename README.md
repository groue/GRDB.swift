...
A database pool allows concurrent reading and writing. Writes are serialized, so that there is no conflict. Reads are isolated, which means that [you can](#databasepool-concurrency) perform several fetch requests in a row without being affected by eventual concurrent writes.
