// Request is the type of query interface requests, generated by TableRecord:
//
//     struct Player: TableRecord { ... }
//     let playerRequest = Player.all() // Request<Player>
//
...
/// Request is a query interface request that generates SQL for you.
///
/// For example:
///
///     try dbQueue.read { db in
...
extension Request: SelectionRequest {
...
extension Request: FilteredRequest {
    // MARK: Request Derivation
...
extension Request: OrderedRequest {
    // MARK: Request Derivation
...
extension Request: AggregatingRequest {
    // MARK: Request Derivation
...
extension Request: _JoinableRequest {
    /// :nodoc:
...
extension Request: JoinableRequest where T: TableRecord { }
...
extension Request {
    ...
    /// Request does not use any `LIMIT 1` sql clause when you
    ...
    public func asRequest<RowDecoder>(of type: RowDecoder.Type) -> Request<RowDecoder> {
        Request<RowDecoder>(query: query)
    }
}
...
extension Request: TableRequest {
    /// :nodoc:
    public var databaseTableName: String {
        ...
        case .query:
            ...
            //
            // This query is currently never wrapped in a Request
...
extension Request: DerivableRequest where T: TableRecord { }

extension Request where T: MutablePersistableRecord {
...
private func prefetch(_ db: Database, associations: [SQLAssociation], in rows: [Row]) throws {
    ...
    // CAUTION: Keep this code in sync with Request.databaseRegion(_:)
    for association in associations {
        ...
        do {
            ...
            let prefetchedRelation = association
                ...
                .annotated(with: pivotColumns.map { pivotAlias[Column($0)].forKey("grdb_\($0)") })
            prefetchedRows = try Request(relation: prefetchedRelation)
                .fetchAll(db)
