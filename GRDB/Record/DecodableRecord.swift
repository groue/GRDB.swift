...
public protocol DecodableRecord {
    ...
    /// When the DecodableRecord type also adopts the standard Decodable
    ...
    ///
    ///     // A DecodableRecord + Decodable record
...
extension DecodableRecord where Self: Hashable {
...
extension DecodableRecord where Self: Hashable {
    ...
    public static func fetchSet<R: FetchRequest>(_ db: Database, _ request: R) throws -> Set<Self> {
        ...
        if let supplementaryFetch = request.supplementaryFetch {
            let rows = try Row.fetchAll(request.statement, adapter: request.adapter)
            try supplementaryFetch(db, rows)
            var set = Set<Self>(minimumCapacity: rows.count)
            for row in rows {
                try set.insert(Self(row: row))
            }
            return set
        } else {
...
extension FetchRequest where RowDecoder: DecodableRecord & Hashable {
...
/// `DatabaseDateDecodingStrategy` specifies how `DecodableRecord` types that
