...
public struct LayoutedColumnMapping {
    ...
    ///
    ///     // [foo:"foo" bar: "bar"]
    ///     try Row.fetchOne(db, rawSQL: "SELECT NULL, 'foo', 'bar'", adapter: FooBarAdapter())
...
///
///     // [baz:3]
///     try Row.fetchOne(db, rawSQL: sql, adapter: adapter)
public protocol RowAdapter {
    ...
    ///
    ///     // [foo:1]
    ///     try Row.fetchOne(db, rawSQL: "SELECT 1, 2, 3", adapter: FirstColumnAdapter())
    func layoutedAdapter(from layout: RowLayout) throws -> LayoutedRowAdapter
}
...
public struct ScopeAdapter : RowAdapter {
    ...
    ///     let row = try Row.fetchOne(db, rawSQL: "SELECT 1, 2, 3", adapter: adapter)!
    ///     row                  // [1, 2, 3]
