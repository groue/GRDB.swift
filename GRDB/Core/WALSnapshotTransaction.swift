...
final class WALSnapshotTransaction: @unchecked Sendable {
    // @unchecked because `databaseAccess` is protected its mutex.
    ...
    /// nil when closed
    @Mutex private var databaseAccess: DatabaseAccess?
    ...
    func read<T>(_ value: (Database) throws -> T) throws -> T {
        try $databaseAccess.withLock { databaseAccess in
    ...
    func asyncRead(_ value: @escaping @Sendable (Result<Database, Error>) -> Void) {
        $databaseAccess.withLock { databaseAccess in
            guard let databaseAccess else {
