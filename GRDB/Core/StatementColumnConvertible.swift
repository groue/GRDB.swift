...
///
///     let rows = Row.fetchCursor(db, rawSQL: "SELECT ...")
///     while let row = try rows.next() {
///         let int: Int = row[0]                 // there
///     }
///     let ints = Int.fetchAll(db, rawSQL: "SELECT ...") // there
///     struct Player {
///         init(row: Row) {
///             name = row["name"]                // there
...
///         let names: ColumnCursor<String> = try String.fetchCursor(db, rawSQL: "SELECT name FROM player")
...
///
///     try dbQueue.read { db in
///         let emails: NullableColumnCursor<String> = try Optional<String>.fetchCursor(db, rawSQL: "SELECT email FROM player")
...
extension DatabaseValueConvertible where Self: StatementColumnConvertible {
    ...
    /// - returns: A cursor over fetched values.
    /// - throws: A DatabaseError is thrown whenever an SQLite error occurs.
    public static func fetchCursor(_ db: Database, literal sqlLiteral: SQLLiteral, adapter: RowAdapter? = nil) throws -> FastDatabaseValueCursor<Self> {
...
extension Optional where Wrapped: DatabaseValueConvertible & StatementColumnConvertible {
    ...
    /// - returns: A cursor over fetched optional values.
    /// - throws: A DatabaseError is thrown whenever an SQLite error occurs.
    public static func fetchCursor(_ db: Database, literal sqlLiteral: SQLLiteral, adapter: RowAdapter? = nil) throws -> FastNullableDatabaseValueCursor<Wrapped> {
        return try fetchCursor(db, SQLRequest<Void>(literal: sqlLiteral, adapter: adapter))
    }
