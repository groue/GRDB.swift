...
extension Database {
    ...
    private static func validateSQLCipher(_ sqliteConnection: SQLiteConnection) throws {
        ...
        // > connection.
        var _sqliteStatement: SQLiteStatement? = nil
        let code = sqlite3_prepare_v2(sqliteConnection, "PRAGMA cipher_version", -1, &_sqliteStatement, nil)
        guard code == SQLITE_OK else {
            throw DatabaseError(resultCode: code, message: String(cString: sqlite3_errmsg(sqliteConnection)))
        }
        defer {
        ...
        if sqlite3_step(_sqliteStatement) != SQLITE_ROW || (sqlite3_column_text(_sqliteStatement, 0) == nil) {
