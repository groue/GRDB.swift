...
/// ## Overview
///
/// A `DatabaseValueConvertible` type supports conversion to and from
/// database values (null, integers, doubles, strings, and blobs).
/// `DatabaseValueConvertible` is adopted by `Bool`, `Int`, `String`,
/// `Date`, etc.
///
/// > Note: Types that converts to and from multiple columns in a database
/// > row must not conform to the `DatabaseValueConvertible` protocol. Those
/// > types are called **record types**, and should conform to record
/// > protocols instead. See <doc:QueryInterface>.
///
/// ## Conforming to the DatabaseValueConvertible Protocol
///
/// To conform to `DatabaseValueConvertible`, implement the two requirements
/// ``fromDatabaseValue(_:)-21zzv`` and ``databaseValue-1ob9k``. Do not
/// customize the ``fromMissingColumn()-7iamp`` requirement. Do not
/// customize the conformance of `Optional`: it is built-in when the wrapped
/// type is itself `DatabaseValueConvertible`.
///
/// It is important that the implementation of `fromDatabaseValue` returns
/// nil if the type can not be decoded from the raw database value. This
/// nil value will have GRDB throw a decoding error accordingly.
///
/// For example:
///
/// ```swift
...
///
/// `DatabaseValueConvertible` implementation is ready-made for
/// `RawRepresentable` types whose raw value is itself
/// `DatabaseValueConvertible`, such as enums:
///
/// ```swift
/// enum Grape: String {
...
///
/// `DatabaseValueConvertible` is also ready-made for `Codable` types, which
/// are automatically coded and decoded from JSON arrays and objects:
///
/// ```swift
...
/// ```
///
/// By default, such codable value types are encoded and decoded with the
/// standard [JSONDecoder](https://developer.apple.com/documentation/foundation/jsondecoder)
/// and [JSONEncoder](https://developer.apple.com/documentation/foundation/jsonencoder).
/// `Data` values are handled with the `.base64` strategy, `Date` with the
/// `.millisecondsSince1970` strategy, and non conforming floats with the
/// `.throw` strategy.
///
/// In order to customize the JSON format, provide a custom implementation
/// of the `DatabaseValueConvertible` requirements.
///
/// ### Adding support for the Tagged library
///
/// [Tagged](https://github.com/pointfreeco/swift-tagged) is a popular
/// library that makes it possible to enhance the type-safety of our
/// programs with dedicated wrappers around basic types. For example:
///
/// ```swift
/// import Tagged
///
...
/// ```
///
/// Applications that use both Tagged and GRDB will want to add those lines
/// somewhere:
///
/// ```swift
/// import GRDB
...
/// ```
///
/// This makes it possible to use `Tagged` in all the expected places:
///
/// ```swift
/// let id: Player.ID = ...
...
/// ## Optimized Values
///
/// For extra performance, custom value types can conform to both
/// `DatabaseValueConvertible` and ``StatementColumnConvertible``. This
/// extra protocol grants raw access to the [low-level C SQLite
/// interface](https://www.sqlite.org/c3ref/column_blob.html) when
/// decoding values.
///
/// For example:
///
...
/// ```
///
/// > Note: Standard collections `Array`, `Set`, and `Dictionary` do not
/// > conform to `DatabaseValueConvertible`, on purpose. You won't be able
/// > to directly fetch or store arrays, sets, or dictionaries as JSON
/// > database values.
/// >
/// > Standard collections get free JSON support when they are embedded as
/// > properties of `Codable` record types, though: see <doc:QueryInterface>.
/// >
/// > It is not advised to consider the absence of those conformances as a
/// > lack, and add `DatabaseValueConvertible` conformance to those
/// > collection types in your application. This would litter JSON values in
/// > unexpected places, and foster misuse. For example, it is better when
