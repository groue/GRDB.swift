...
extension FetchRequest where T: RowConvertible {
    ...
    public func fetchAll(reader: DatabaseReader) -> [T] {
        return reader._readWithSingleStatementIsolation { db in Array(self.fetch(db)) }
    }
    ...
    public func fetchOne(reader: DatabaseReader) -> T? {
        return reader._readWithSingleStatementIsolation { db in self.fetch(db).generate().next() }
    }
}
...
extension DatabaseValueConvertible {
    ...
    public static func fetchAll<T>(reader: DatabaseReader, _ request: FetchRequest<T>) -> [Self] {
        return reader._readWithSingleStatementIsolation { db in try! fetchAll(request.selectStatement(db)) }
    }
    ...
    public static func fetchOne<T>(reader: DatabaseReader, _ request: FetchRequest<T>) -> Self? {
        return reader._readWithSingleStatementIsolation { db in try! fetchOne(request.selectStatement(db)) }
    }
}
